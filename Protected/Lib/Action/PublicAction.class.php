...
class PublicAction extends Action {
    public function main() {
        ...
        //留言信息
        $message=D('Message');
        $lynum=$message->where("from_unixtime(create_time,'%Y-%m-%d')=CURDATE()")->count();//今日流量数
        $lynum1=$message->where("from_unixtime(create_time,'%Y%m%d')=CURDATE()-1")->count();//昨日流量数
        $lynum2=$message->where("from_unixtime(create_time,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')")->count();//当月流量数
        
        //订单信息
        $Order=D('Order');
        $ddnum=$Order->where("from_unixtime(create_time,'%Y-%m-%d')=CURDATE()")->count();//今日流量数
        $ddnum1=$Order->where("from_unixtime(create_time,'%Y%m%d')=CURDATE()-1")->count();//昨日流量数
        $ddnum2=$Order->where("from_unixtime(create_time,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')")->count();//当月流量数
...
//        $list=$tjurl->order('create_num desc')->limit(10)->select();
        ...
        $this->lynum=$lynum==null ? 0:$lynum;
        $this->lynum1=$lynum1==null ? 0:$lynum1;
        $this->lynum2=$lynum2==null ? 0:$lynum2;
