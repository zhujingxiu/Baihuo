...
<body class="bodybg">
    ...
    <dd class="left-l-tab">
        <ul class="tab-list">
            <?php $_list=D('Category')->where('status=1 and id in (37,38,39)')->field("*,concat(path,'-',id) as bpath")->order('bpath')->select();foreach($_list as $key=>$value):$_list[$key]['count']=count(explode('-',$value['bpath']));endforeach;$_result=$_list;foreach($_result as $key=>$category): if($_result[$key]['count'] == $_result[$key-1]['count'] or $_result[$key-1]['count'] == null): ?><li <?php if($category['id'] == I('id')): ?>class="tabon"<?php endif; ?>><a href="<?php echo U('Content/index',array('id'=>$category['id']));?>"><?php echo ($category["catname"]); ?></a></li> 
                <?php elseif($_result[$key]['count'] > $_result[$key-1]['count']): ?>
                    <ul>
                    <li <?php if($category['id'] == I('id')): ?>class="tabon"<?php endif; ?>><a href="<?php echo U('Content/index',array('id'=>$category['id']));?>"><?php echo ($category["catname"]); ?></a></li>
                <?php elseif($_result[$key]['count'] < $_result[$key-1]['count']): ?>
                    </ul>
                    <li <?php if($category['id'] == I('id')): ?>class="tabon"<?php endif; ?>><a href="<?php echo U('Content/index',array('id'=>$category['id']));?>"><?php echo ($category["catname"]); ?></a></li><?php endif; endforeach;?>
        </ul>
    </dd>
</dl>
        </div>
        	...
        	<div class="content">
                ...
                <div class="content_main">
                    <ul class="listcontent">
                        ...
                        <div class="img"><a href="<?php echo U('Product/show',array('id'=>$vo['id']));?>" target="_blank"><img src="__ROOT__/Uploads/<?php echo ($vo['thumb']); ?>" width="160" height="160" /></a></div>
                        <div class="title"><a href="<?php echo U('Product/show',array('id'=>$vo['id']));?>" target="_blank"><?php echo (msubstr($vo['title'],0,30)); ?></a></div>
                        <div class="price">价格：<span>￥<?php echo ($vo['price']); ?></span></div>
                        </li><?php endforeach; endif; else: echo "" ;endif; ?>
                    </ul>
                </div>
                <div class="clear"></div>
